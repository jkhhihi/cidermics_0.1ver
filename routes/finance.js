...
router.post('/finance/apply/insert', function(req, res, next) {
	...
	var fi_app_sex = req.body.mem_sex;
	...
	var row;
	var sets = {fi_app_cate : fi_app_cate, fi_app_name : fi_app_name, fi_app_phone : fi_app_phone, fi_app_email : fi_app_email, fi_app_sex:fi_app_sex, fi_app_job : fi_app_job, fi_app_path : fi_app_path, fi_app_comment : fi_app_comment, fi_app_regDate : date, fi_app_upDate : date, fi_app_age:fi_app_age, fi_app_place:fi_app_place};
...
router.get('/finance/review', function(req, res, next) {
	...
	mysql.select("SELECT COUNT(*) AS appno FROM cider.cid_fi_applyform where fi_app_cate="+fi_app_cate+";", function (err, data){
		...
		mysql.select('SELECT * FROM cider.cid_fi_review order by rev_no desc;', function (err, data2){
			...
			mysql.select('SELECT rev_title, rev_intDate FROM cider.cid_fi_review where rev_intDate = '+rd+' order by rev_no desc;', function (err, data3){
		
	res.render('front/cid_finance/cid_finance_review', {finance : data, rev:data2, rev2:data3, rd:rd});
	});
   });
 });
});
...
router.get('/finance/s', function(req, res, next) {

	res.render('front/cid_survey/cid_finance_s', { });

});


router.post('/finance/s', function(req, res, next) {
